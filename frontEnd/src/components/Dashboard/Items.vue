  ...
  <div>
    <div class="filters-container pb-7">
      <div class="filters-column">
        ...
        <button
          @click="selectedStatus = 'purchased'"
          class="button select"
    ...
    <template v-if="selectedStatus === 'private'">
      <v-data-table
        :headers="headers"
        :items="tablePrivateItems"
        item-value="value"
        item-text="label"
        class="elevation-1"
        :search="search"
        :loading="privateItems.isLoading"
        loading-text="Loading... Please wait"
        @click:row="handleOpenItem"
      ></v-data-table>
    </template>
    <template v-if="selectedStatus === 'purchased'">
      <v-data-table
        :headers="headers"
        :items="tablePurchasedItems"
        item-value="value"
        item-text="label"
        class="elevation-1"
        :search="search"
        :loading="purchasedItems.isLoading"
        @click:row="handleOpenItem"
      ></v-data-table>
    </template>
    ...
    >
      <v-card>
        <v-card-title
          class="headline"
        >
          <span v-if="uploadStep === 1">Upload dataset</span>
          <span v-if="uploadStep === 2">Successfully uploaded</span>
...
<script>
  ...
  import { getDashboardItems, uploadItem, uploadDataset } from '@/api_client.js';
  ...
  export default {
    ...
    methods: {
      handleFileUpload(event) {
        ...
        uploadItem(this.file.name, this.file)
          .then(response => {
            this.uploadStep += 1;
      ...
      handleFileSubmit() {
        this.isLoading = true;
        uploadDataset(this.datasetName, this.fileId)
          .then(({ data }) => {
    ...
    created() {
      ...
      getDashboardItems('purchased')
        .then(({ data }) => this.purchasedItems.data = data)
